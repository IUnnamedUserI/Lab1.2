...
void CreateControlScreen()
{
    ...
    Control.CreateButton("Инструктор", "Instructor");
}
...
void CreateInstructorScreen()
{
    ...
    InstructorMenu.CreateButton("Вкл/выкл Инструктор", "OnOffInstructor");
...
public Color GetParam(string Param)
{
    ...
    for (int i = 0; i < Data.Count(); i++)
        ...
        {
            string[] ParamSplit = Data[i].Split(':');
            if (ParamSplit[1] == "Вкл.") return Color.Green; else return Color.Red;
        }
    return Color.Red;
}
...
class ContextMenu
{
    ...
    {
        Button Btn = new Button(Name, new Vector2(65f, 100 + 40 * ButtonList.Count), Tag);
        ButtonList.Add(Btn);
    ...
    public Button SelectedButton() { return ButtonList[SelectedButtonID]; }
}
...
class Button
{
    ...
    public Button(string Text, Vector2 Position, string Tag)
    {
        this.Text = Text;
    ...
    public List<MySprite> DrawButton()
    {
        ...
        ButtonList.Add(ButtonSprite);
        ButtonSprite = new MySprite(SpriteType.TEXT, Text, Position, null, Color.Green, "Debug", TextAlignment.LEFT, 1.5f);
        ButtonList.Add(ButtonSprite);
